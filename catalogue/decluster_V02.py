...
# set new header
newheader = lines[0].strip() #+ ',pref_mw, ml2mw_ble, ml2mw_qde, ml2mw_qds'
newtxt = newheader
...
for line in lines[1:]:
    ...
    if np.isnan(float(dat[16])):
        ...
        eqidx = np.where((datestr == dat[0]) \
                         ...
                         & (mx_origML >= checkfloat(dat[16])-0.1) & (mx_origML <= checkfloat(dat[16])+0.1))[0]
    ...
    if len(eqidx) > 0:
        ...
        if prefmag2 == 'mw':
            try:
                newline = ','.join(dat[0:16]) + ',' + str('%0.2f' % prefmag[eqidx][0]) + ',,MW,' \
                          + ','.join((str('%0.2f' % mx_origML[eqidx][0]), mx_origType[eqidx][0], \
                                      str('%0.2f' % mx_revML[eqidx][0]), str('%0.2f' % prefmag[eqidx][0]), \
                                      str('%0.2f' % ml2mw_ble[eqidx][0]), str('%0.2f' % ml2mw_qde[eqidx][0]), \
                                      str('%0.2f' % ml2mw_qds[eqidx][0]))) + '\n'
        ...
        # keep orig mag in "magnitude" column
            ...
            newline = ','.join((line.strip(), str('%0.2f' % mx_origML[eqidx][0]), mx_origType[eqidx][0], \
                                str('%0.2f' % mx_revML[eqidx][0]), str('%0.2f' % prefmag[eqidx][0]))) + '\n'
