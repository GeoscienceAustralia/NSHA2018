...
catalogue = parser.read_file(start_year=start_year, end_year=end_year)
# Note this doesn't seem to deal with the time interval properly,
# so now we purge events outside of the time range
# How many events in the catalogue?
print "The catalogue contains %g events" % catalogue.get_number_events()
index = np.logical_and(catalogue.data['year'] >= start_year, catalogue.data['year'] <= end_year)
#print index
...
completeness_table_a = np.array([[1975., 3.0]])
...
#print smoother_grid
#print max(smoother_grid[3])
#print max(smoother_grid[4])
...
#    print smoother.data[j,:]
    ...
    point = Point(data[j,0],data[j,1],
                data[j,2])
    # Rates 
    annual_rate = data[j,4]/(end_year - start_year + 1)
    aval =  np.log10(annual_rate) + bval*completeness_table_a[0][1]
...
#print data[:,4]
basemap1.m.scatter(x, y, marker = 's', c = np.log10(smoother.data[:,4] - bvalue*completeness_table_a[0][1]), cmap = plt.cm.coolwarm, zorder=10, lw=0,
                   vmin=-7.0, vmax = -1.0 )
...
plt.savefig('smoothed_%i_%i_%.1f_b%.3f_0.1.png' % (
