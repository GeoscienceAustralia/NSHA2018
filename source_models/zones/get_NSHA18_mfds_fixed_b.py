'''
This code is for fixed b-values from gridded b-value model
'''
...
###############################################################################
# now, assign zone rates with fixed b-values
###############################################################################
...
###############################################################################
# loops through individual sources using smoothed b
###############################################################################
...
for i in srcidx:
    ...
    fixed_beta = bval2beta(src_bval_fix[i])
    fixed_sigbeta = bval2beta(src_bval_fix_sd[i])    
    ...
    if len(mvect) <= 1:
        ...
        if skipPlotting == True and float(src_class[i]) <= 8.0:
            ...
            plt_width = 0.1
            beta = bval2beta(bval)
            betacurve, mfd_mrng = get_oq_incrementalMFD(fixed_beta, N0areanorm, mpltmin_best, mrng[-1], plt_width)
            
            # calculate rate of M5 events
            rates = N0areanorm * exp(-fixed_beta  * mfd_mrng) * (1 - exp(-fixed_beta * (src_mmax[i] - mfd_mrng))) \
                    / (1 - exp(-fixed_beta * src_mmax[i]))
    ...
    ###############################################################################
        ...
        print(str(src_bval_fix[i])+' fixed_beta: '+str(fixed_beta))
        ...
        # get a-value using region class b-value for other sources
            ...
            fn0 = fit_a_value(src_bval_fix[i], mrng, cum_rates, src_mmax[i], bin_width, midx)
        ...
        fixed_sigbeta173 = 1.73 * fixed_sigbeta
        fixed_sigb173 = 1.73 * src_bval_fix_sd[i]
        ...
        if not isnan(bval):
            ...
            # non-normalised rates
                ...
                # get lower + 1 std
                bc_tmp, bc_mrng_lo = get_oq_incrementalMFD(fixed_beta+fixed_sigbeta, dummyN0, mpltmin, src_mmax_l[i], bin_width)
                # fit to err_lo
                bc_lo100 = (cum_rates[midx][0] - err_lo[midx][0]) * (bc_tmp / bc_tmp[0])
                # solve for N0
                N0_lo100 = 10**(log10(bc_lo100[0]) + beta2bval(fixed_beta+fixed_sigbeta)*bc_mrng_lo[0])
                
                # get lower + 1.73 std
                bc_tmp, bc_mrng_lo = get_oq_incrementalMFD(fixed_beta+fixed_sigbeta173, dummyN0, mpltmin, src_mmax_l[i], bin_width)
                # fit to err_lo
                bc_lo173 = (cum_rates[midx][0] - err_lo[midx][0]) * (bc_tmp / bc_tmp[0])
                # solve for N0
                N0_lo173 = 10**(log10(bc_lo173[0]) + beta2bval(fixed_beta+fixed_sigbeta173)*bc_mrng_lo[0])
                
                # get upper - 1 std
                bc_tmp, bc_mrng_up = get_oq_incrementalMFD(fixed_beta-fixed_sigbeta, dummyN0, mpltmin, src_mmax_u[i], bin_width)
                # fit to err_up
                bc_up100 = (cum_rates[midx][0] + err_up[midx][0]) * (bc_tmp / bc_tmp[0])
                # solve for N0
                N0_up100 = 10**(log10(bc_up100[0]) + beta2bval(fixed_beta-fixed_sigbeta)*bc_mrng_up[0])
                
                # get upper - 1.73 std
                bc_tmp, bc_mrng_up = get_oq_incrementalMFD(fixed_beta-fixed_sigbeta173, dummyN0, mpltmin, src_mmax_u[i], bin_width)
                # fit to err_up
                bc_up173 = (cum_rates[midx][0] + err_up[midx][0]) * (bc_tmp / bc_tmp[0])
                # solve for N0
                N0_up173 = 10**(log10(bc_up173[0]) + beta2bval(fixed_beta-fixed_sigbeta173)*bc_mrng_up[0])
        ...
        new_bval_b[i] = src_bval_fix[i]
        #new_bval_l[i] = bval-sigb173
        #new_bval_u[i] = bval+sigb173
        
        # Use +/- 1 sigma
        new_bval_l[i] = src_bval_fix[i]+src_bval_fix_sd[i] # lower curve, so higher b
        new_bval_u[i] = src_bval_fix[i]-src_bval_fix_sd[i] # upper curve, so lower b
        ...
        if doPlots == True:
            ...
            if not isnan(bval):
                ...
                betacurve, mfd_mrng = get_oq_incrementalMFD(fixed_beta, fn0, mpltmin_best, mrng[-1], plt_width)
                ...
                up173_txt = '\t'.join(('Upper 1.73x', str('%0.3e' % N0_up173), str('%0.2f' % (fixed_beta-fixed_sigbeta173)), \
                                       str('%0.3f' % beta2bval(fixed_beta-fixed_sigbeta173)), str('%0.1f' % src_mmax_u[i]))).expandtabs()
                                       
                up100_txt = '\t'.join(('Upper 1.00x', str('%0.3e' % N0_up100), str('%0.2f' % (fixed_beta-fixed_sigbeta)), \
                                       str('%0.3f' % beta2bval(fixed_beta-fixed_sigbeta)), str('%0.1f' % src_mmax_u[i]))).expandtabs()
                                       
                best_txt  = '\t'.join(('Best Estimate', str('%0.3e' % fn0), str('%0.2f' % fixed_beta), \
                                       str('%0.3f' % beta2bval(fixed_beta)), str('%0.1f' % src_mmax[i]))).expandtabs()
                
                lo100_txt = '\t'.join(('Lower 1.00x', str('%0.3e' % N0_lo100), str('%0.2f' % (fixed_beta+fixed_sigbeta)), \
                                       str('%0.3f' % beta2bval(fixed_beta+fixed_sigbeta)), str('%0.1f' % src_mmax_l[i]))).expandtabs()
                                       
                lo173_txt = '\t'.join(('Lower 1.73x', str('%0.3e' % N0_lo173), str('%0.2f' % (fixed_beta+fixed_sigbeta173)), \
                                       str('%0.3f' % beta2bval(fixed_beta+fixed_sigbeta173)), str('%0.1f' % src_mmax_l[i]))).expandtabs()
