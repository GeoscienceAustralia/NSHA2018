...
outshp = path.join('shapefiles','ARUP_NSHA18_M4.shp')
...
for i, shape in enumerate(shapes):
    ...
    # write new records
    if i >= 0:
        w.record(name[i], codes[i], src_ty, neotec_num[i], src_wt, dep_b[i], dep_u[i], dep_l[i], min_mag, min_rmag, mmax[i], mmax[i]-0.2, mmax[i]+0.2, \
