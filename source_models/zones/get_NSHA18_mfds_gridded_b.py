...
for i in srcidx:
    ...
    mvect, mxvect, tvect, dec_tvect, ev_dict \
        = get_events_in_poly(i, sourcecat, polygons[i], polygons, src_usd, src_lsd, src_overwrite_lsd)
    print(len(ev_dict))    
    # get area (in km**2) of sources for normalisation
    src_area.append(get_WGS84_area(polygons[i]))
    ...
    if len(mvect) < 50:
        ...
        # Use +/- 1 sigma
        new_bval_l[i] = nan # lower curve, so higher b
        new_bval_u[i] = nan # upper curve, so lower b
    ...
    ###############################################################################
        ...
        bval, beta, sigb, sigbeta, fn0, cum_rates, ev_out, out_idx, err_up, err_lo = \
              ...
              src_bval_fix[i], src_bval_fix_sd[i], 0.1, polygons[i])
        
        # get annualised rates based on preferred MW (mvect)
        tmp_cum_rates, cum_num, bin_rates, n_obs, n_yrs = \
        ...
        # Use +/- 1 sigma
        new_bval_l[i] = bval+sigb # lower curve, so higher b
        new_bval_u[i] = bval-sigb # upper curve, so lower b
        ...
        if doPlots == True and isnan(bval) == False:
            ...
            plt.plot(orig_dec_tvect[didx], orig_mvect[didx], 'bo', label=Nfail+' Failed')
            
            # replot those that pass completeness
            didx = where(tvect > dcut)[0]
            plt.plot(dec_tvect[didx], mvect[didx], 'ro', label=Npass+' Passed')
            ...
            if not isnan(bval):
                ...
                betacurve, mfd_mrng = get_oq_incrementalMFD(beta, fn0, mpltmin, mrng[-1], plt_width)
            ...
            for mr in range(0,len(mrng)):
                ...
                area_norm_beta_curve_val =  beta_curve_val
...
# map b-value
###############################################################################
# reload original shape for plotting
del sf
sf = shapefile.Reader(shpfile)
...
for b, n0 in zip(new_bval_b, new_n0_b):
    if not isnan(n0) and not isnan(b):
...
drawshapepoly(m2, plt, sf, cindex=cindex, cmap=cmap, ncolours=ncolours, fillshape=True, alpha=1.)
...
bmap = path.join(rootfolder,modelsplit+'_b_val_map.png')
plt.savefig(bmap, format='png', bbox_inches='tight')
plt.gcf().clear()
plt.clf()
plt.close()
